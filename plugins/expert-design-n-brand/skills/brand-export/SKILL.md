...
> **Canonical vocabulary source.** Token names, role labels, surface slot names, and CSS/Tailwind/Figma keys referenced throughout this skill — and in every artifact it produces — are defined in `assets/platform-matrix-template.md`. If anything here appears to disagree with the Matrix, the Matrix wins.
...
3. **Contract validation** against `build-export-contract.md`:
   - Walk every Required path — missing = hard-fail with: "Brand system missing {field}. Run /brand-build to populate before re-trying export." Cite the contract section.
   - Walk every Required path — same hard-fail behavior. **Do not auto-compose at export time**; that's brand-build's job. If `personality.character`, `archetype_in_action[]`, `do_dont`, `color.roles[]`, `section_summaries.*`, `typography.*.character`, `typography.*.alternatives[]`, `typography.per_platform.*`, `governance.owner.name`, or `platforms[]` are missing, block and direct user to re-run brand-build.
   - For alternate input shapes (`palette` as 4-role object, flat `anchors` dict, etc.), normalize in-memory — do NOT modify the source YAML. Log each normalization as INFO in the verification report.
...
## Per-artifact compliance checklist (mandatory before declaring an artifact done)

Before moving on from any artifact (i.e. before step 10 above logs it as complete), walk this checklist explicitly. The structural validator (step 8) catches *shape* problems; this checklist catches *content* problems the validator can't see.

For every artifact, before logging:

1. **Placeholder completion sweep.** Walk every `{{ placeholder }}` slot and every `<!-- REPEAT:name -->` block in the source template. For each one, confirm the rendered file has a non-empty filled value OR the slot is explicitly marked optional in `rendering-rules.md` (e.g. `<!-- OPTIONAL:name -->` blocks may be dropped when their driving technique was not run). If any required slot is empty, fix it now — do not log success with empty slots.

2. **External-facing vocabulary scan.** Read the rendered output as a checklist task against `rendering-rules.md`'s external-facing principle. Reject and rewrite any occurrence of plugin-internal words a reader has no context for — `plugin`, `skill`, `phase`, `retrofit`, `PASS / WARN / FAIL`, `${CLAUDE_PLUGIN_ROOT}`, `brand-build`, `brand-export`, slash commands like `/brand-export`. Tier 1 artifacts are forward-looking reference docs; the reader has never heard of this tool.

3. **Section-pair completeness check.** For every section in a Tier 1 artifact, confirm both `<p class="section-definition">` (universal copy describing what the section IS) AND `<p class="lead">` or `<p class="section-summary">` (brand-specific answer) exist. The template enforces this structurally; if a section is missing either paragraph, the template is wrong — stop, fix the template, and re-render.

4. **Cross-artifact token-name consistency.** Every token name appearing in this artifact must match the exact string used in `tokens.json` `semantic.{light,dark}.color.*` and in every sibling artifact (`design-system.yaml`, `quick-ref.md`, `brand-reference.html`, `brand.md`, `playground.html`). Run a fast grep against the sibling artifacts already on disk; any string-mismatch is a fail.

5. **L/D parity for color sections.** If the artifact has a color section (e.g. `brand-guidelines.html` §Color, `quick-ref.md` Color table, `playground.html`), confirm light and dark blocks each list all 14 canonical color-role slots (`bg`, `surface`, `surface-elevated`, `inverse`, `text-{primary,secondary,tertiary,disabled,inverse}`, `border`, `border-{strong,focus}`, `link`, `link-hover`) per `skills/design-system/references/token-architecture.md`. A missing slot in either mode is a fail. Single-mode artifacts (e.g. print PDFs with a light-only color section) evaluate this gate as N/A.

6. **Essence ≠ archetype.** If the artifact has a Personality section, confirm `essence.name` and `archetype_primary` are rendered as distinct fields in adjacent cards, NEVER concatenated into a hyphenated or spaced compound label. See `rendering-rules.md` Rules 54–66.

7. **Hex-source check.** Every hex value in the artifact must trace back to `palette.core[]`, `palette.semantic[]`, or `palette.anchors[]` in `brand-identity.yaml`. Hex values that don't appear in any of those lists are not declared anchors — flag them and replace with a declared hex.

State explicitly at the end of each artifact's render:

```
Compliance checklist for {artifact}: PASS
  - Placeholders: N filled, M optional-skipped
  - Forbidden vocab: 0 hits
  - Section pairs: complete
  - Cross-artifact token consistency: verified against {list-of-siblings-checked}
  - L/D parity: light=14/14, dark=14/14 (or N/A for non-color artifacts)
  - Essence ≠ archetype: verified (or N/A)
  - Hex-source: all values traced to palette
```

OR, on any failure:

```
Compliance checklist for {artifact}: FAIL
  - {gate}: {reason}, fixing now
```

Then fix and re-check. Do not log a `PASS` to `export_log[]` if any gate failed — log the failure honestly so the user can decide whether to re-render.

This is a deliberate read-back-and-self-correct pass — a reading task with this list as a checklist, not a regex hook.
...
- **Contract-first**: no Required field is auto-composed at export time. Missing fields block export with a message pointing to brand-build.
